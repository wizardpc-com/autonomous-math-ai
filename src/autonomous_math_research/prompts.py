...
def director_prompt(
    ...
    *,
    ...
    prompt = (
        ...
        "ClaimGraph and controller receipts alone are authority. Audited Frontier is routing only: "
        "it may suppress an exact external audited scope pending authority. Model output cannot change trust, "
        "evidence, representation compatibility, audit verdicts, or lifecycle state.\n\n"
        "SCHEDULING RULES\n"
        ...
        "No unverified bridge into trusted final claims. Apply the Campaign Theme, dependency "
        "closure, closed/duplicate removal, and exact kill gates in order. Never leave the Theme "
        "for an unrelated gap. Query relevant assets before proposing a lemma, bridge, generator, "
        "checker, or tool; reuse an equivalent audited asset or state exact non-applicability. "
        "Treat an unregistered declared core_terms "
...
def worker_prompt(
    ...
    return (
        ...
        f"{render_contract_keys(WORKER_RESULT_KEYS)}. For every asset loaded in "
        "task_packet.research_context, include exactly one asset_usage row: USED "
